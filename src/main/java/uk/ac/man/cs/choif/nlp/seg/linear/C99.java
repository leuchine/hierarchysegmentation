...
import uk.ac.man.cs.choif.nlp.statistics.distribution.Acc;
...
/**
 * C99 algorithm for linear text segmentation Creation date: (11/05/99 03:30:09)
 * 
 * @author: Freddy Choi
 */
public class C99 {
	private final static class Region {
		...
		/**
		 * For the region find a boundary that maximises the inside density of
		 * the region. The result is stored in the region itself.
		 * 
		 * @param M
		 *            float[][]
		 * @param R
		...
		public final static void bestBoundary_max(final float[][] M, Region R) {
			...
			if (start < end) {
				/*
				 * For each possible boundary, compute the density to find the
				 * best boundary
				 */
				int B = start;
				float D_max = 0, D = 0;
				for (int i = end, lower = end - start, upper = 1; i-- > start; lower--, upper++) {
					...
					D = (M[i][start] + M[end][i + 1])
							/ (float) ((lower * lower) + (upper * upper));
	 ...
	 *            float[][]
	 * @param n
	 *            int Number of regions to find, if -1, the algorithm will
	 *            decide.
	 */
	private final static int[] boundaries(final float[][] M, final int n) {
		/*
		 * A list of regions that can be split. Initialise it with the entire
		 * document as the first region.
		 */
		Vector R = new Vector(); // List of splitable regions
		Region r = new Region(0, M.length - 1, M); // The entire document as the
													// first region
		Region.bestBoundary_max(M, r); // Find the best split point
		R.addElement(r); // Initialise R with the entire document

		/*
		 * Split the document right down to elementary units, i.e. sentences.
		 * The boundary selected at each step of this divisive clustering
		 * process is placed in a list B. Each step increases the inside
		 * density, this increase is placed in a list dD.
		 */

		int[] B = new int[M.length - 1]; // A list of boundary locations
		float[] dD = new float[M.length - 1]; // Increase in density due to the
												// split
		...
		for (int i = 0, ie = (n != -1 ? n - 1 : M.length - 1); i < ie; i++) {
			/*
			 * Find the best region to split. It is one that maximizes the
			 * overall density
			 */
			...
			for (int j = R.size(); j-- > 0;) {
				...
				if (r.start < r.end) {
					...
					tempD = (sum_region - r.sum + r.left.sum + r.right.sum)
							/ (float) (sum_area - r.area + r.left.area + r.right.area);
					/* Maximize */
					if (tempD > maxD) {
						maxD = tempD;
						index = j;
			...
			Region.bestBoundary_max(M, r.left); // Find best split point for
												// left region
			Region.bestBoundary_max(M, r.right); // Find best split point for
													// right region
		...
		/*
		 * Now that we have an ordered list of boundary locations, the next
		 * problem is to decide the number of segments to have. Granularity is
		 * task dependent. The following method computes # segments by tracking
		 * the change in density caused by each split.
		 */
		...
		if (n != -1) {
			...
			/* Determine # segments */
			dD = Statx.convolve(dD, new float[] { 1, 2, 4, 8, 4, 2, 1 }); // Smooth
																			// gradient
			Acc dist = new Acc(); // Capture distribution of gradient
			dist.acc(dD);
			double threshold = dist.mean() + 1.2 * dist.sd(); // Threshold to
																// capture only
																// unusually
																// large
																// gradient
	...
	/**
	 * Segment a piece of text using the C99 algorithm Creation date: (11/05/99
	 * 06:05:36)
	 * 
	 * @param args
	 *            java.lang.String[]
	...
	public final static void main(String[] args) {
		...
		try (DirectoryStream<Path> stream = Files.newDirectoryStream(dir)) {
			for (Path file : stream) {
				if (file.getFileName().toString().endsWith("ref.text")) {
					ArrayList<String> lines = new ArrayList<String>();
					Scanner scan = new Scanner(new File(file.getParent()
					...
					String[] D = new String[objs.length];
					...
					String[][] S = segment(D, k);
				
					PrintWriter pw = new PrintWriter(file.getParent()
							.toString() + "/" + file.getFileName() + ".out");
	...
	/**
	 * Given a document as a list of sentences, this function produces a list of
	 * stem frequency tables, or context vector Creation date: (11/05/99
	 * 03:43:34)
	 * 
	...
	/**
	 * Apply hard ranking (replace pixel value with the proportion of
	 * neighbouring values its greater than) to matrix using a S x S mask.
	...
	private final static float[][] rank(final float[][] F, final int S) {
		...
		for (int M_i = M.length; M_i-- > 0;) {
			for (int M_j = M_i + 1; M_j-- > 0;) {
				 ...
				 * because we ignore the middle pixel which will always be rank
				 * 0.
				 */
	...
	/**
	 * Given a document as a list of elementary text blocks (usually sentences),
	 * segment the document into n coherent topic segments. If n is -1, the
	 * algorithm will decide the appropriate number of segments by monitoring
	 * the rate of increase in segment density. Creation date: (11/05/99
	 * 05:55:46)
	 * 
	 * @return String[][] A list of coherent topic segments
	 * @param String
	 *            [] A list of elementary text blocks (usually sentences). Each
	 *            block is a string of space separated tokens.
	 * @param n
	 *            int Number of segments to make, if -1 then let the algorithm
	 *            decide.
	 */
	public final static String[][] segment(String[] document, int n) {
		...
		float[][] sum = sum(rank);
		rank = null;
		Debugx.msg("C99", "Divisive clustering ("
				+ (n == -1 ? "automatic" : "user") + " termination)...");
	...
	/**
	 * Given a list fo context vector, compute the similarity matrix Creation
	 * date: (11/05/99 04:45:51)
	 * 
	 * @return float[][]
	 * @param v
	 *            uk.ac.man.cs.choif.extend.structure.ContextVector[]
	 */
	...
	/**
	 * Given the input text and the topic boundaries, split the text into
	 * segment blocks. Creation date: (08/16/99 06:56:26)
	 * 
	...
	private final static String[][] split(final String[] T, final int[] B) {
		...
		int[] b = new int[B.length + 2]; // A list of boundaries, includes
											// implicit boundaries
		b[0] = 0;
	...
	/**
	 * Compute the sum of rank matrix Creation date: (11/05/99 04:56:32)
	 * 
	 * @return float[][]
	 * @param M
	 *            float[][]
	...
	private final static float[][] sum(final float[][] M) {
		...
		for (int j = 2, ij, ip; j < M.length; j++) {
			for (int i = 0, ie = M.length - j; i < ie; i++) {
				ij = i + j;
				ip = i + 1;
				S[ij][i] = M[ij][i] * 2 + S[ij - 1][i] + S[ij][ip]
						- S[ij - 1][ip];
				S[i][ij] = S[ij][i];
			}
		}
